...
FLAVIA_DIRECTORY = "../flavia"
FLAVIA_LABELS = "../flavia_labels.csv"
#IMG_HEIGHT = 256
#IMG_WIDTH = 256
...
def square_flavia(img):
    
    pad_xl = int((img.shape[1] - img.shape[0])/2)
    pad_xr = img.shape[1] - img.shape[0] - pad_xl
   
    img2 = cv2.copyMakeBorder(img,pad_xl,pad_xr,0,0,cv2.BORDER_CONSTANT,value=(255,255,255)) 
 
    return img2


def load_image(size, segment=False):
    print('Load pictures')
...
def load_image_test(size, segment=False):
    print('Load pictures')
...
def load_flavia(size, segment=False):
    with open(FLAVIA_LABELS,'r') as f:
        labels_txt = f.read().split('\n')
    label_list = {}   
    for l in labels_txt:
        idx = l.split(',')[0]
        val = l.split(',')[1].split('-')
        label_list[int(idx)] = {'begin':int(val[0]) , 'end':int(val[1])}
   
    
    print('Load Flavia pictures')
    path_to_all_files = FLAVIA_DIRECTORY + '\\*.jpg' 
    files = glob(path_to_all_files)
    ...
    for img_file in files:
        idx = int(img_file.split(split_motif)[-1].split('.')[0])
        img = square_flavia(cv2.imread(img_file))
        label = 0
        for i in label_list.keys():
            if idx >= label_list[i]['begin'] and idx <= label_list[i]['end']:
                label = i
                break
        if label == 0:
            print('problem with class')

        trainImg.append(cv2.resize(img, (size, size)))  # Get image (with resizing)    
        trainLabel.append(label) 
    trainImg = np.asarray(trainImg)  # Train images set
    trainLabel = pd.DataFrame(trainLabel)  # Train labels set

    print('Preprocess flavia pictures')
    clearTrainImg = []
    if segment == True:    
        for img in trainImg:
            image = sharpen_image(segment_plant(img))
...
if __name__== "__main__":   
    ...
    for size in TEST_SIZE:
        trainImg, clearTrainLabel = load_flavia(size)
        ...
        num_classes = len(clearTrainLabel[0])
        print(num_classes)
        
        base_model = VGG19(weights= None, include_top=False, input_shape= (size,size,3))
        ...
        model.summary()     
        stem_id = "9_1_1_VGG19-trainnet-mask-flavia-imgsize"
        ...
        # checkpoints
        stem_id = "9_1_1_VGG19-unfrozen2-mask-flavia-imgsize"
        filepath=stem_id+str(size)+".best_{epoch:02d}-{val_accuracy:.2f}.hdf5"
        checkpoint = ModelCheckpoint(filepath, monitor='val_accuracy', 
                                     verbose=1, save_best_only=True, mode='max')
        filepath=stem_id+str(size)+".last_auto4.hdf5"
        ...
        test_img = load_image_test(size, segment=True)  
