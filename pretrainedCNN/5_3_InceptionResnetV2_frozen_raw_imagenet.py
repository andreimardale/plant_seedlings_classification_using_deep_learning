...
if __name__== "__main__":   
    ...
    TEST_SIZE = [75, 128, 256, 512]
        
    
    for size in TEST_SIZE:
        print('### Use Pict size ' + str(size))
        trainImg, clearTrainLabel = load_image(size, segment=False)
        ...
        base_model = InceptionResNetV2(weights= 'imagenet', include_top=False, input_shape= (size,size,3))
        ...
        # checkpoints
        stem_id = "5_3_InceptionResnetV2-frozen-raw-imagenet-imgsize"
        ...
        test_img = load_image_test(size, segment=False)  
        Y_test_pred = model.predict(test_img, batch_size=BATCH_SIZE)
        Y_classes = Y_test_pred.argmax(axis=-1)
        with open(stem_id + str(size) + '_test_ypred.csv', 'w', newline='') as myfile:
            wr = csv.writer(myfile)
            wr.writerow(Y_classes)                              
